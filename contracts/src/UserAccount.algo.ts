...
import {
  ...
  // Increase limit after verification
  this.limit.value = this.limit.value + 50;
   }

   @abimethod()
   addToPiggyBank(amount: uint64): void {
     assert(Txn.sender === this.ownerAddress.value, 'Only owner can add to piggy bank');
     this.piggyBank.value = this.piggyBank.value + amount;
   }

   @abimethod()
   getPiggyBankBalance(): uint64 {
     return this.piggyBank.value;
   }
